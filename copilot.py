...
PROXIMITY_THRESHOLD = st.sidebar.slider("Proximidade Suporte (%)", 0.1, 2.0, 0.5) / 100
...
def check_signal(df, symbol):
    if df.empty or len(df) < SUPPORT_RESISTANCE_PERIOD: return None

    current = df.iloc[-1]
    support_level = current['support']
    current_price = current['close']
    is_currently_in_proximity = False

    if support_level > 0 and current_price <= support_level * (1 + PROXIMITY_THRESHOLD) and current_price >= support_level * (1 - PROXIMITY_THRESHOLD):
        is_currently_in_proximity = True

    # Enviar alerta SE o preço ESTIVER na faixa de proximidade
    if is_currently_in_proximity:
        message = f"""
        ⚠️ *ALERTA DE PROXIMIDADE DO SUPORTE - {symbol}* ⚠️
        ⏳ Timeframe: {TIMEFRAME}
        💰 Preço Atual: {current_price:.2f}
        🛡️ Nível de Suporte: {support_level:.2f}
        🤏 Proximidade: {(current_price - support_level) / support_level * 100:.2f}%
        ⏰ {datetime.now().strftime('%d/%m %H:%M')}
        """
        send_telegram_alert(message) # Envia o alerta sempre que estiver na proximidade
...
def create_chart(df, symbol):
    ...
    fig.update_layout(
        title=f"{symbol} | {TIMEFRAME} | Último: {df['close'].iloc[-1]:.2f}" if not df.empty else f"{symbol} | {TIMEFRAME} | Carregando...",
        ...
        with placeholder.container():
            ...
            for symbol in SYMBOLS:
                if symbol in all_data:
                    ...
                    if signals.get(symbol) and signals[symbol]['type'] == 'BUY':
                        st.success(f"""
